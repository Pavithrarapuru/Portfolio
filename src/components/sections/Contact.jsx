...
function Contact() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error('Failed to save message to AWS')
      }
  ...
  return (
    <section id="contact" data-label="Contact" className="mx-auto grid max-w-[1140px] grid-cols-[.9fr_1.1fr] items-start gap-16 px-6 py-16 max-tablet:grid-cols-1 max-tablet:gap-10 max-tablet:px-4 max-tablet:py-12">
      ...
      <Reveal
        className="rounded-xl bg-surface p-8 shadow-raised max-tablet:p-6 neo-transition"
        delay={0.15}
      >
        <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
          ...
          <AnimatePresence>
            {status.message && (
              <motion.p
                className={`mt-2 text-center font-mono text-[11px] font-semibold ${status.type === 'success' ? 'text-accent' : 'text-red-500'
                  }`}
